    ...
    <ion-accordion-group>
        <ion-accordion >
            ...
            <div slot="content" class="ion-padding">
                <ion-chip v-if="meta.payment_card_fixate_sum>0" color="primary">
                    <ion-icon :src="cardOutline"/>
                    <ion-text>Блок {{meta.payment_card_fixate_sum}}{{$heap.state.currencySign}}</ion-text>
                </ion-chip>
                <ion-chip v-if="meta.payment_card_refund_sum>0" color="primary">
                    <ion-icon :src="cardOutline"/> 
                    <ion-text>Разблок. {{meta.payment_card_refund_sum}}{{$heap.state.currencySign}}</ion-text>
                </ion-chip>
                <ion-chip v-if="meta.payment_card_confirm_sum>0" color="primary">
                    <ion-icon :src="cardOutline"/>
                ...
                <ion-button v-if="meta.invoice_link" color="light" @click="billOpen(meta.invoice_link)">
                    <ion-icon :src="receiptOutline" slot="start"/>
                    <ion-text>Открыть чек {{meta.invoice_sum}}{{$heap.state.currencySign}}</ion-text>
                </ion-button>
